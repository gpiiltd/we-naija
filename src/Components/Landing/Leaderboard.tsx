      ...
      <div className="p-6 w-full relative z-10">
            ...
            {Object.keys(tabLabels).map((tab) => (
              <button
                ...
                className={`px-4 py-4 rounded-md text-sm font-medium ${
                  activeTab === tab ? "bg-[#ED7D31] text-white shadow" : "bg-white text-gray-600"
                }`}
              >
        ...
        <div className="max-w-5xl mx-auto rounded-2xl bg-white overflow-hidden">
              ...
              <tbody>
                {leaderboardData[activeTab].map((user, index) => (
                  <tr key={index} className="border-b last:border-none">
                    ...
                    <td className="px-6 py-4 text-[#ED7D31] font-bold">{user.points} SP</td>
          ...
          <div className="md:hidden p-4">
            {leaderboardData[activeTab].map((user, index) => (
              <Card key={index} className="flex items-center justify-between p-6 shadow-md mb-2">
                ...
                <div className="text-[#ED7D31] font-bold text-sm">{user.points} SP</div>
              </Card>
            ))}
          </div>
        </div>
...
const Card = ({ children, className = "" }: React.PropsWithChildren<{ className?: string }>) => (
  <div className={`bg-white w-full rounded-xl shadow-md ${className}`}>{children}</div>
);
